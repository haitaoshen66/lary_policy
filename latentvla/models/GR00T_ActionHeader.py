...
from latentvla.models.flow_matching_head.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)
from latentvla.models.flow_matching_head.cross_attention_dit import DiT
...
def build_flow_gr00t_action_head(
    vlm_hidden_size: int,
    action_dim: int,
    action_horizon: int,
    state_dim: int,
    ...
    if flow_dit_size not in size_map:
        raise ValueError(f"Unsupported flow_dit_size: {flow_dit_size}")

    action_model_cfg = {
        "action_model_type": size_map[flow_dit_size],
    ...
    full_config = SimpleNamespace(
        framework=SimpleNamespace(
            action_model=SimpleNamespace(**action_model_cfg)
        )
    )
    return get_action_model(config=full_config)
