...
class GenerateConfig:
    ...
    wandb_project: str = "vla_libero"        # Name of WandB project
...
from latentvla.models import constants
VLA_CONSTANT_OVERRIDES = {
    "la_align": {
        "NUM_TOKENS": 56,
...
def check_unnorm_key(cfg: GenerateConfig, model) -> None:
    ...
    logger.info(f"Using action un-norm key: {unnorm_key}")
...
def setup_logging(cfg: GenerateConfig):
    ...
    # Create run ID
    run_id = f"EVAL-{cfg.task_suite_name}-{cfg.vla_id}-{DATE_TIME}"
...
# import latentvla.models.constants as constants
...
#     },
# }
def _propagate_constant_overrides(overrides: dict) -> None:
    """Update already-imported latentvla modules with new constant values."""
    ...
    for module_name, module in list(sys.modules.items()):
        if not module_name or module is None:
            continue
        if not module_name.startswith(("latentvla.", "exp.")):
            continue
...
def _apply_constant_overrides(constants_module, overrides: dict) -> None:
    """Apply overrides on the constants module and propagate to dependents."""
    ...
    _propagate_constant_overrides(sanitized)
